...
updated: '2024-08-07 18:37:42'
---

[Subroutines chapter]: http://docs.parrot.org/parrot/latest/html/docs/book/pir/ch06_subroutines.pod.html

It's time to treat the star database like a database. Well, it's time to treat it like something we can search, anyways. I know this is not a trivial task in Parrot, so the Babysteps have been building up to it slowly. In [Parrot Babysteps 06 Files and Hashes]({{< relref "/post/2009/10/parrot-babysteps-06-files-and-hashes.md" >}}), we figured out how to read from the database file and display its contents in a meaningful fashion. [Parrot Babysteps 07 Writing Subroutines]({{< relref "/post/2009/10/parrot-babysteps-07-writing-subroutines.md" >}}) added subroutines to massage the data a little and produce some usable names for the thousands of stars in the database that do not have proper names. Then we suddenly spun off in a seemingly random direction with [Parrot Babysteps 08 Testing with Testmore]({{< relref "/post/2009/12/parrot-babysteps-08-testing-with-testmore.md" >}}) and [Parrot Babysteps 09 Simple Projects]({{< relref "/post/2010/04/parrot-babysteps-09-simple-projects.md" >}}).

That was intentional, though.

This sort of project requires more work in [Parrot]({{< relref "/card/parrot.md" >}}) than in a language like Perl, due to Parrot's lower-level nature. I wanted to be sure we could test this application as we add search functionality. We just pushed our script into the [Parrot Babysteps 0a the Stellar Project]({{< relref "/post/2010/06/parrot-babysteps-0a-the-stellar-project.md" >}}) for a testing foundation.
